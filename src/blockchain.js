...
const MAXTIMEDIFF = 300;  // Max time diff to process message in second
...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (current_time - method < MAXTIMEDIFF) {
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
           let block = self.chain.filter(block => block.hash == hash.toString())[0]
           if (block) {
            resolve(block);
           } else {
            reject(null);
